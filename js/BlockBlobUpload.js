...
var sendFile = function (blockLength) {
    ...
    sendNextChunk = function () {
        ...
        jqxhr = $.ajax({
            async: true,
            url: ('/ajaxupload/UploadBlock.php?id=' + incrimentalIdentifier),
            data: fileChunk,
            cache: false,
            contentType: false,
            processData: false,
...
function startUpload(fileElementId, blockLength, uploadProgressElement, statusLabel, uploadButton, cancelButton, assetId, container) {
    ...
    var dataString = 'blocksCount=' + uploader.totalBlocks + '&fileName=' + uploader.file.name +
        '&fileSize=' + uploader.file.size + '&container=' + container + '&assetId=' + assetId;
    $.ajax({
        type: "POST",
        async: true,
        url: '/ajaxupload/PrepareMetadata.php',
        data: dataString,
        error: function () {
...
function testfunc() {
    $.ajax({
        ...
        async: true,
        url: '/ajaxupload/PrepareMetadata.php',
        data: 'blocksCount=100&fileName=FILETEST&fileSize=200&container=TESTCON&assetId=ASID',
        error: function () {
            alert(operationType.METADATA_FAILED);
        },
        success: function (operationState) {
            if (operationState === true) {
                alert('success');
